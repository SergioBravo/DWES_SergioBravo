<?php
  ...
  function comprarProducto($cliente,$producto,$conn) {
    try {
      ...
      actualizarStockAlmacen($cliente,$producto,$conn);
    }
    catch(PDOException $e) {
  ...
  function actualizarStockAlmacen($cliente,$producto,$conn) {//Pasamos un cliente, un producto y un almacen a usar y compramos dicho producto en caso de que haya stock
  ...
  function verCarrito($carrito,$conn) {//Visualizamos el contenido de la cesta del cliente
    ...
    if ($size == 0) {
      ...
      echo "<ul>";
        for ($i=0; $i < $size; $i++) {
          echo "<li>".sacarNombreProducto($carrito[$i],$conn)."</li>";
        }
      echo "</ul>";
    }
  }
